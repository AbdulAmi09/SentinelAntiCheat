...
def _maia_limit() -> chess.engine.Limit:
    ...
    if settings.maia_nodes > 0:
        kwargs["nodes"] = settings.maia_nodes
    if settings.maia_time_seconds > 0:
        kwargs["time"] = settings.maia_time_seconds
...
class MaiaPolicyContext:
    ...
    def move_probability(self, board: chess.Board, move: chess.Move) -> float | None:
        # engine.analyse() only surfaces the *last* UCI "info string" line, which
        # for lc0's --verbose-move-stats is a root-aggregate summary with no move
        # notation attached. The per-legal-move policy lines we actually need are
        # emitted throughout the search, so this must consume the full stream via
        # engine.analysis() rather than a single analyse() snapshot.
        strings: list[str] = []
        try:
            with self.engine.analysis(board, _maia_limit(), info=chess.engine.INFO_ALL) as analysis:
                for info in analysis:
                    line = info.get("string")
                    if isinstance(line, str):
                        strings.append(line)
                    elif isinstance(line, list):
                        strings.extend(s for s in line if isinstance(s, str))
        except Exception as exc:
            LOGGER.warning("Maia policy analyse failed: %s", exc)
            return None

        policy = _parse_policy(strings)
        if not policy:
            return None
